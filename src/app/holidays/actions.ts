...
/**
 * sendHolidayCancellationEmailAction - Notifies manager when a request is cancelled by the employee.
 */
export async function sendHolidayCancellationEmailAction({
  managerEmail,
  employeeName,
  startDate,
  endDate,
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #334155; line-height: 1.6;">
          <div style="background: #64748b; padding: 25px; text-align: center; border-radius: 8px 8px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 24px;">Leave Cancelled</h1>
          </div>
          <div style="padding: 30px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 8px 8px;">
            <p>Hello,</p>
            <p><strong>${employeeName}</strong> has cancelled their <strong>${type}</strong> request which was previously <strong>${status}</strong>.</p>
            
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 8px; margin: 25px 0;">
              <p style="margin: 0; font-size: 14px; color: #64748b; text-transform: uppercase; font-weight: bold;">Cancelled Dates</p>
              <p style="margin: 5px 0 0 0; font-size: 14px;"><strong>From:</strong> ${new Date(startDate).toLocaleDateString()}</p>
              <p style="margin: 5px 0 0 0; font-size: 14px;"><strong>Until:</strong> ${new Date(endDate).toLocaleDateString()}</p>
            </div>

            <p>The project schedule and employee leave balance have been updated accordingly.</p>
