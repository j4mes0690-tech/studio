...
import type { PlannerTask, Trade } from '@/lib/types';
import { 
    format, 
    addDays, 
    startOfDay, 
                                    ...
                                    {isVisible && (
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <div 
                                                    className={cn(
                                                        "absolute h-8 top-2 rounded-md shadow-sm border-2 flex items-center px-2 transition-all group-hover:shadow-md",
                                            ...
                                            <TooltipContent className="p-3">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-xs">{trade?.name}</p>
                                                    <p className="text-[10px] opacity-80">{format(taskStart, 'PP')} &rsaquo; {task.durationDays} Days</p>
                                                    <Separator className="my-1" />
                                                    <p className="text-[10px] font-bold uppercase">Status: {task.status}</p>
                                                </div>
                                            </TooltipContent>
                                        </Tooltip>
                                    )}
