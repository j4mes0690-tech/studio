...
export function RespondToRequest({ item, currentUser }: RespondToRequestProps) {
        ...
        const uploadedPhotos = await Promise.all(
          photos.map(async (p, i) => {
            if (p.url.startsWith('data:')) {
              ...
              const url = await uploadFile(storage, `rfi-threads/photos/${Date.now()}-${i}.jpg`, blob);
              return { ...p, url };
            }
            return p;
          })
        ...
        const uploadedFiles = await Promise.all(
          files.map(async (f, i) => {
            if (f.url.startsWith('data:')) {
              ...
              const url = await uploadFile(storage, `rfi-threads/files/${Date.now()}-${i}-${f.name}`, blob);
              return { ...f, url };
            }
            return f;
          })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Discuss technical details or provide clarifications.
          </DialogDescription>
        </DialogHeader>
                    ...
                    return (
                        <div key={msg.id} className={cn("flex flex-col", isMe ? "items-end" : "items-start")}>
                            <div className={cn(
                                ...
                                {msg.photos?.map((p, i) => (
                                  <div key={i} className="relative aspect-video rounded overflow-hidden border bg-background mt-2 min-w-[100px]">
                                    <Image src={p.url} alt="Attached" fill className="object-cover" />
                                  </div>
                                ))}
                                {msg.files?.map((f, i) => (
                                  <div key={i} className={cn("flex items-center gap-1.5 p-1.5 rounded text-[10px] border mt-2", isMe ? "bg-primary-foreground/10 border-primary-foreground/20 text-white" : "bg-background border-border text-primary")}>
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                ...
                render={({ field }) => (
                    ...
                    <FormControl>
                        <Textarea 
                            placeholder="Add your response or technical clarification..." 
                  ...
                  {isCameraOpen ? (
                    <div className="flex-1 space-y-2 border rounded p-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={() => setFacingMode(p => p === 'user' ? 'environment' : 'user')}><RefreshCw className="h-3 w-3" /></Button>
