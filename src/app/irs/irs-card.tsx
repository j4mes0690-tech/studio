...
export function IRSCard({ 
  ...
  return (
    <Card className={cn(
        ...
        item.status === 'provided' ? 'border-l-green-500 opacity-75' : 'border-l-primary'
    )}>
      ...
      <CardContent className="space-y-4">
        ...
        <Button 
            variant={item.status === 'provided' ? 'secondary' : 'outline'} 
            className="w-full h-9 gap-2 font-bold"
            onClick={handleToggleProvided}
            disabled={isPending}
        >
            {item.status === 'provided' ? <RefreshCw className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
            {item.status === 'provided' ? 'Reopen Requirement' : 'Mark as Provided'}
        </Button>
      </CardContent>
    </Card>
  );
