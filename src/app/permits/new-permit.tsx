          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="p-6 space-y-6">
                ...
                <FormField control={form.control} name="description" render={({ field }) => (
                    <FormItem>
                        ...
                        <FormControl><Textarea placeholder="Specific details of the task..." className="min-h-[100px]" {...field} /></FormControl><FormMessage />
                    </FormItem>
                )} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField control={form.control} name="validFrom" render={({ field }) => (
                        <FormItem><div className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /><FormLabel>Valid From</FormLabel></div><FormControl><Input type="datetime-local" {...field} /></FormControl></FormItem>
                    )} />
                    <FormField control={form.control} name="validTo" render={({ field }) => (
                        <FormItem><div className="flex items-center gap-2"><Clock className="h-4 w-4 text-destructive" /><FormLabel>Expiry Time</FormLabel></div><FormControl><Input type="datetime-local" {...field} /></FormControl></FormItem>
                    )} />
                </div>
                ...
                <div className="space-y-4">
                    <FormLabel>Documentation & Media</FormLabel>
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button type="button" variant="outline" className="w-24 h-24 flex flex-col gap-1 border-dashed" onClick={() => fileInputRef.current?.click()}><Upload className="h-6 w-6" /><span className="text-[10px]">Upload</span></Button>
                    </div>
                    {isCameraOpen && (
            ...
            <DialogFooter className="p-6 bg-muted/30 border-t gap-3">
              ...
              <Button type="submit" className="w-full sm:flex-1 h-12 text-lg font-bold" disabled={isPending} onClick={() => form.setValue('status', 'issued')}>
                {isPending && submissionStatus === 'issued' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-5 w-5" />}
                Issue Permit
              </Button>
            </DialogFooter>
