...
import { Camera, PlusCircle, Loader2, Save, Upload, X, Building2, MapPin, Sparkles } from 'lucide-react';
...
export function AddPhotoDialog({ projects, currentUser }: { projects: Project[], currentUser: DistributionUser }) {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      setCapturedPhoto({ url: optimized, takenAt: new Date().toISOString() });
    };
  ...
  const onSubmit = (values: AddPhotoFormValues) => {
    if (!capturedPhoto) {
        toast({ title: 'Photo Missing', description: 'Capture or upload a progress photo first.', variant: 'destructive' });
        return;
    }

    startTransition(async () => {
      try {
        toast({ title: 'Uploading', description: 'Persisting image to site registry...' });

        const blob = await dataUriToBlob(capturedPhoto.url);
        ...
        await addDoc(collection(db, 'site-photos'), photoData);
        toast({ title: 'Success', description: 'Progress photo recorded.' });
        setOpen(false);
      } catch (err) {
        console.error(err);
        toast({ title: 'Error', description: 'Failed to record documentation.', variant: 'destructive' });
      }
    });
  };
          ...
          <DialogHeader className="p-6 pb-4 bg-primary/5 border-b shrink-0">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <DialogDescription>Record high-fidelity visual evidence for the site audit trail.</DialogDescription>
                </div>
            </div>
          </DialogHeader>

          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit, () => scrollToFirstError())} className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="space-y-4">
                      ...
                      {capturedPhoto ? (
                          <>
                              ...
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                  ...
                                  <Button type="button" variant="destructive" size="sm" className="font-bold" onClick={() => setCapturedPhoto(null)}><X className="h-4 w-4 mr-2" /> Remove</Button>
                              </div>
                          </>
                      ) : (
                          <div className="flex flex-col items-center gap-4 p-8 text-center">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">Use the controls below to start documentation</p>
                              </div>
                          </div>
                      )}
                  </div>
                  ...
                  <div className="flex gap-2">
                      <Button type="button" variant="outline" className="flex-1 h-11 gap-2 font-bold" onClick={() => setIsCameraOpen(true)}>
                          <Camera className="h-4 w-4" /> Camera
                      </Button>
                      <Button type="button" variant="outline" className="flex-1 h-11 gap-2 font-bold" onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField control={form.control} name="projectId" render={({ field }) => (
                          <FormItem>
                              ...
                              <Select onValueChange={(v) => { field.onChange(v); form.setValue('areaId', 'none'); }} value={field.value}>
                  ...
                  <FormField control={form.control} name="description" render={({ field }) => (
                      <FormItem>
                          ...
                          <FormControl><Input placeholder="e.g. Completed foundation pour..." {...field} className="h-10" /></FormControl>
                          <FormMessage />
                      </FormItem>
                  )} />
                </div>
